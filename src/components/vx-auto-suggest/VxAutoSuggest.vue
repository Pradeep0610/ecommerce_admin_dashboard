...
<div class="vx-auto-suggest">
    ...
    <ul ref="scrollContainer" class="auto-suggest-suggestions-list z-50 rounded-lg mt-2 shadow-lg overflow-hidden" :class="{'hidden': !inputFocused}" @mouseenter="insideSuggestions = true" @mouseleave="insideSuggestions = false" @focus="updateInputFocus" @blur="updateInputFocus(false)" tabindex="-1">
        <li ref="option" class="auto-suggest__suggestion flex items-center justify-between py-3 cursor-pointer" v-for="(suggestion, index) in filteredData" :key="index" :class="{'vx-auto-suggest__current-selected': currentSelected == index, 'pointer-events-none': suggestion.index < 0}" @mouseenter="currentSelected = index" @click="suggestionSelected">
