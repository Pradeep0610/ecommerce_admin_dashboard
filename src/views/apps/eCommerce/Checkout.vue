                ...
                <div class="vx-col lg:w-2/3 w-full">
                    <vx-card title="Add New Address" subtitle="Be sure to check &quot;Deliver to this address&quot; when you have finished" class="mb-base">
                        <ValidationObserver v-slot="{passes}">
                            <form data-vv-scope="add-new-address" @submit.prevent="passes(submitNewAddressForm)">
                                <div class="vx-row">
                                    <div class="vx-col sm:w-1/2 w-full">
                                        <validation-provider name="Full Name" rules="required|alpha_spaces" v-slot="{ valid, errors }">
                                            <vs-input data-vv-as="field" name="fullName" label="Full Name:" v-model="fullName" class="w-full mt-5" />
                                            <span v-show="errors" class="text-danger">{{ errors[0] }}</span>
                                        </validation-provider>
                                    </div>

                                    <div class="vx-col sm:w-1/2 w-full">
                                        <validation-provider name="Mobile Number" rules="required|digits:10" v-slot="{ valid, errors }">
                                            <vs-input rules="'required|digits:10'" name="mobileNum" label="Mobile Number:" v-model="mobileNum" class="w-full mt-5" />
                                            <span v-show="errors" class="text-danger">{{ errors[0] }}</span>
                                        </validation-provider>
                                    </div>
                                </div>
                                ...
                                <div class="vx-row">
                                    <div class="vx-col sm:w-1/2 w-full">
                                        <validation-provider name="House Number" rules="required" v-slot="{ valid, errors }">
                                            <vs-input rules="'required'" name="houseNum" label="Flat, House No:" v-model="houseNum" class="w-full mt-5" />
                                            <span v-show="errors" class="text-danger">{{ errors[0] }}</span>
                                        </validation-provider>
                                    </div>

                                    <div class="vx-col sm:w-1/2 w-full">
                                        <vs-input name="landmark" label="Landmark e.g. near apollo hospital:" v-model="landmark" class="w-full mt-5" />
                                    </div>
                                </div>

                                <div class="vx-row">
                                    <div class="vx-col sm:w-1/2 w-full">
                                        <validation-provider name="City" rules="required" v-slot="{ valid, errors }">
                                            <vs-input name="city" label="Town/City:" v-model="city" class="w-full mt-5" />
                                            <span v-show="errors" class="text-danger">{{ errors[0] }}</span>
                                        </validation-provider>
                                    </div>

                                    <div class="vx-col sm:w-1/2 w-full">
                                        <validation-provider name="Pincode" rules="required|min:3|max:6|numeric" v-slot="{ valid, errors }">
                                            <vs-input name="pincode" label="Pincode:" v-model="pincode" class="w-full mt-5" />
                                            <span v-show="errors" class="text-danger">{{ errors[0] }}</span>
                                        </validation-provider>
                                    </div>
                                </div>
                                ...
                                <div class="vx-row">
                                    <div class="vx-col sm:w-1/2 w-full">
                                        <validation-provider name="State" rules="required" v-slot="{ valid, errors }">
                                            <vs-input name="state" label="State:" v-model="state" class="w-full mt-5" />
                                            <span v-show="errors" class="text-danger">{{ errors[0] }}</span>
                                        </validation-provider>
                                    </div>
                                ...
                                <vs-button class="mt-6 ml-auto flex">SAVE AND DELIVER HERE</vs-button>
                            </form>
                        </ValidationObserver>
                    </vx-card>
                </div>
                        ...
                        <div class="mt-3">
                            <ul>
                                ...
                                <li>
                                    ...
                                    <!-- CVV BLOCK -->
                                    <ValidationObserver v-slot="{passes}">
                                        <form data-vv-scope="cvv-form" class="mt-4" @submit.prevent="passes(makePayment)">
                                            <validation-provider name="cvv" rules="required|digits:3" v-slot="{ valid, errors }">
                                                <div class="flex items-center">
                                                    <span>Enter CVV:</span>
                                                    <vs-input rules="'required|digits:3'" name="cvv" v-model="cvv" class="mr-3 ml-2" />
                                                    <vs-button @click.prevent="makePayment" :disabled="false">CONTINUE</vs-button>
                                                </div>
                                                <span v-show="errors" class="text-danger ml-24">{{ errors[0] }}</span>
                                            </validation-provider>
                                        </form>
                                    </ValidationObserver>
                                </li>
